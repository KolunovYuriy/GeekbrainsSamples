...
(
    select films.name           "name",
           ...
           avg(r3.ticket_count)::text "client_average",
           sum(r2.amount_money) "total_money"
    from films
...
union
(
    select 'Итого', count(t.*), '' , sum(s.price) from schedule s inner join tickets t on s.id = t.schedule_id
)